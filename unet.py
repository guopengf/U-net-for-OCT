...
def center_crop(layer, target_size1,target_size2):
    _, _, layer_width, layer_height = layer.size()
    #thia step make W,H can be different
    start1 = (layer_width - target_size1) // 2
    start2 = (layer_height - target_size2) // 2
    crop = layer[:, :, start1:(start1 + target_size1), start2:(start2 + target_size2)]
    return crop

def concatenate(link, layer):
    #thia step make W,H can be different
    crop = center_crop(link, layer.size()[2],layer.size()[3])
